...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        if animalNumber >= 1:
            ...
            print("so the behaviors extracted are:\n", behavioralEventsForOneAnimal)
        if animalNumber >= 2:
        ...
        for bin in range(nbTimebins):
            ...
            dicoOfBehInfos["Filename"] = fileName[:-14] #Remove the " - Copy.sqlite" part
            date, Xp, cage, LMT, age = fileName[:-14].split("_")  # Splits the name at '_'
            print(f"{date} , {Xp}, {cage}, {LMT}, {age}")
            dicoOfBehInfos["Date"] = date
            dicoOfBehInfos["Cage"] = cage
            dicoOfBehInfos["Session"] = LMT
