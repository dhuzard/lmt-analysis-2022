...
def launch():
    ...
    global df, choicetype, animalnumber, drop_event
    ...
    return df, date, injection, cage, night_phase, event

def create_widgets(injection, cage, event, night_phase):

    global plot_reset, stat_reset, animalnumber, choicetype, statstype, drop_injection_plot, drop_cage_plot, drop_event, drop_night_plot, drop_stat, drop_injection_stat, drop_cage_stat, drop_night_stat, nom_dossier, statbutton
    ...
    return plot_reset, stat_reset, animalnumber, choicetype, statstype, drop_injection_plot, drop_cage_plot, drop_event, drop_night_plot, drop_stat, drop_injection_stat, drop_cage_stat, drop_night_stat, nom_dossier, statbutton

# Affectation de la fonction 'results_update_stats' au bouton 'statbutton'
def get_plots_stats_button(statbutton, drop_injection_stat, drop_cage_stat, drop_night_stat, drop_event, choicetype,
                           statstype):
...
def tabs(plot_reset, drop_injection_plot, drop_cage_plot, drop_night_plot, drop_event, stat_reset, drop_stat,
         drop_injection_stat, drop_cage_stat, drop_night_stat, statstype, nom_dossier, statbutton, animalnumber,
         choicetype) :

    # Association des widgets aux onglets
    box0 = VBox([animalnumber, choicetype])
    ...
    tab0 = widgets.VBox(children=[box0])
    tab1 = widgets.Tab(children=[box1])
    tab2 = widgets.VBox(children=[box2])
    tab1.children = tab0.children + tab1.children + tab2.children
...
def update_event_options(change, animalnumber, drop_event):
...
def reset_selections_plot(drop_injection_plot, drop_cage_plot, df, drop_night_plot, plot_reset):
    drop_injection_plot.value = ()
    drop_cage_plot.options = df['Cage'].unique()
    drop_night_plot.options = df['Night-Phase'].unique()

    plot_reset.on_click(reset_selections_plot)

# Fonction de mise à jour des sélections de l'onglet 'Stat' après clic du bouton 'stat_reset'
def reset_selections_stats(drop_injection_stat, drop_cage_stat, df, drop_night_stat, stat_reset):
    drop_injection_stat.value = ()
    drop_cage_stat.options = df['Cage'].unique()
    drop_night_stat.options = df['Night-Phase'].unique()

    stat_reset.on_click(reset_selections_stats)

# Pour sélectionner plusieurs Dates
def update_options_plot(change, df, drop_injection_plot, drop_cage_plot, drop_night_plot):
...
def update_options_stat(change, df, drop_injection_stat, drop_cage_stat, drop_night_stat):
...
def update_temp_df(change, df, drop_injection_plot, drop_cage_plot, drop_event, drop_night_plot):
...
def update_stats(date, cage, night_phase, event, choice_type, stats_type):
    '''
    ...
    if choicetype.value == 'Number of events' :
        ...
        if not dfs:
            ...
            if drop_stat.value == "Mixed Model":
                ...
                if statstype.value == "1":
                    two_groups_paired_baseline = dabest.load(data=pivot_df2,
                                                             idx=("Control", "Type1"),
                                                             id_col="Index",
                                                             paired='baseline')
                elif statstype.value == "2":
                    two_groups_paired_baseline = dabest.load(data=pivot_df2,
                                                             idx=("Control", "Type1", "Type2"),
                                                             id_col="Index",
                                                             paired='baseline')
                elif statstype.value == "3":
                    two_groups_paired_baseline = dabest.load(data=pivot_df2,
                                                             idx=("Control", "Type1", "Type2", "Type3"),
                                                             id_col="Index",
                                                             paired='baseline')
...
def results_update_stats(date, cage, night_phase, event, choice_type, stats_type, df, choicetype,
                 drop_event, nom_dossier, drop_stat, statstype):
    '''
    ...
    for r_drop_event in drop_event.options:
        ...
        if drop_stat.value == "Mixed Model":
            ...
            if choicetype.value == 'Number of events' :
                    ...
                    # 2ème plot
                    ax = two_groups_paired_baseline_clk.mean_diff.plot(color_col='new_cage',
                                                                       ax=axs[1])
                    axs[1].legend(title='Cage',
